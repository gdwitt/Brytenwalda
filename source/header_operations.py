...
lhs_operations = [try_for_range,
                  ...
                  store_skill_level, #removed from list by lav
...
#Add the following definitions to the end (!) of header_operations.py coop chief
#COOP BEGIN WSE add the operations from the current version of WSE
...
try_for_active_players = 18 #(try_for_active_players, <cur_player_no>, [<skip_server>]), #Loops through all active players. If [<skip_server>] is set to non-zero, the server player (0) will be skipped.
...
is_vanilla_warband = 998 #(is_vanilla_warband), #Fails only when WSE is running
...
menu_create_new = 2410 #(menu_create_new, <destination>, <text>, [<mesh_name>], [<flags>], [<script_no>], [<script_param>]), #Creates a dynamic menu and stores its id into <destination>. [<script_no>] (-1 for no script) will be called with params 1 = menu_no, 2 = [<script_param>] when the operations block is executed
menu_add_item = 2411 #(menu_add_item, <menu_no>, <text>, [<conditions_script_no>], [<consequences_script_no>], [<script_param>]), #Adds a new menu item to <menu_no>. [<conditions_script_no>] and [<consequences_script_no>] (-1 for no script) will be called with params 1 = <menu_no>, 2 = [<script_param>] when the conditions/consequences blocks are executed
menu_clear_items = 2412 #(menu_clear_items, <menu_no>), #Removes all menu items from <menu_no>
menu_clear_generated = 2413 #(menu_clear_generated), #Removes all dynamic menus
...
str_get_char = 2620 #(str_get_char, <destination>, <string_1>, [<index>]), #Stores the numeric value of the [<index>]th character in <string_1> into <destination>
str_to_num = 2621 #(str_to_num, <destination_fixed_point>, <string_1>, [<use_fixed_point_multiplier>]), #Stores the numeric value of <string_1> into <destination_fixed_point>. Decimal values will be rounded to integers, for more precision set [<use_fixed_point_multiplier>] to non-zero
str_compare = 2622 #(str_compare, <destination>, <string_1>, <string_2>, [<case_insensitive>]), #Stores the relationship between <string_1> and <string_2> into <destination> (-1: s1 < s2, 0: s1 = s2, 1: s1 > s2)
str_split = 2623 #(str_split, <destination>, <string_register>, <string_1>, <delimiter>, [<skip_empty>], [<max>]), #Splits <string_1> using <delimiter> into a range of string registers, starting from <string_register>, storing [<max>] substrings at most (default = unlimited), ignoring empty (zero length) substrings if [<skip_empty>] (default = false). Stores the amount of substrings split into <destination>
str_sort = 2624 #(str_sort, <string_register>, [<count>], [<case_insensitive>], [<descending>]), #Sorts a range of [<count>] string registers starting from <string_register>
str_store_lower = 2625 #(str_store_lower, <string_register>, <string_1>), #Stores the lowercase version of <string_1> into <string_register>
str_store_upper = 2626 #(str_store_upper, <string_register>, <string_1>), #Stores the uppercase version of <string_1> into <string_register>
str_store_trim = 2627 #(str_store_trim, <string_register>, <string_1>, [<trim_mode>]), #Stores the whitespace trimmed version of <string_1> into <string_register>. [<trim_mode>]: 0 (default) = trim leading and trailing, 1 = trim leading, 2 = trim trailing
str_store_replace = 2628 #(str_store_replace, <string_register>, <string_1>, <string_2>, <string_3>), #Stores <string_1> into <string_register>, replacing occurrences of <string_2> with <string_3>
str_store_md5 = 2629 #(str_store_md5, <string_register>, <string_1>), #MD5 encrypts <string_1> and stores it into <string_register>
str_store_substring = 2630 #(str_store_substring, <string_register>, <string_1>, [<start>], [<length>]), #Stores a substring of <string_1> into <string_register>, starting from [<start>]. If [<length>] is not specified, everything on the right of <start> will be used
str_store_reverse = 2631 #(str_store_reverse, <string_register>, <string_1>), #Stores the reverse of <string_register> into <string_1>
str_store_join = 2632 #(str_store_join, <string_register>, <start_string_register>, <count>, [<delimiter>]), #Joins <count> string registers starting from string register <start_string_register>, using [<delimiter>] (default = empty string) and stores them into <string_register>
str_store_replace_spaces_with_underscores = 2633 #(str_store_replace_spaces_with_underscores, <string_register>, <string_1>), #Stores <string_1> into <string_register>, replacing all spaces with underscores
str_store_replace_underscores_with_spaces = 2634 #(str_store_replace_underscores_with_spaces, <string_register>, <string_1>), #Stores <string_1> into <string_register>, replacing all underscores with spaces
str_store_multiplayer_profile_name = 2635 #(str_store_multiplayer_profile_name, <string_register>, <profile_no>), #Stores <profile_no>'s name into <string_register>
str_store_face_keys = 2636 #(str_store_face_keys, <string_register>, <face_key_register>), #Stores the string representation of <face_key_register> into <string_register>
str_store_module_setting = 2637 #(str_store_module_setting, <string_register>, <setting>), #Stores the string value (empty if not found) of <setting> in module.ini into <string_register>
str_store_server_password_admin = 2638 #(str_store_server_password_admin, <string_register>), #Stores the server administrator password into <string_register>
str_store_server_password_private = 2639 #(str_store_server_password_private, <string_register>), #Stores the server private player password into <string_register>
str_store_overlay_text = 2640 #(str_store_overlay_text, <string_register>, <overlay_no>), #Stores <overlay_no>'s text into <string_register>
str_store_player_ip = 2641 #(str_store_player_ip, <string_register>, <player_no>), #Stores <player_no>'s IP address into <string_register> (works only on servers)
str_store_game_variable = 2642 #(str_store_game_variable, <string_register>, <variable>), #Stores the string value (empty if not found) of <variable> in game_variables.txt into <string_register>
str_store_skill_name = 2643 #(str_store_skill_name, <string_register>, <skill_no>), #Stores the name of <skill_no> into <string_register>
str_store_float = 2644 #(str_store_float, <string_register>, <fp_register>, [<precision>]), #Stores the string representation of <fp_register> into <string_register> showing [<precision>] decimal digits at most
#WSE chief
str_sanitize                              = 2645 #(str_sanitize, <string_register>), #Removes invalid characters from <string_register>
str_store_item_id                         = 2646 #(str_store_item_id, <string_register>, <item_no>), #Stores the id of <item_no> into <string_register>
#gdw commenting out repeated variables or numbers
face_keys_init = 2740 #(face_keys_init, <face_key_register>), #Zeroes <face_key_register>
face_keys_copy = 2741 #(face_keys_copy, <face_key_register_1>, <face_key_register_2>), #Copies <face_key_register_2> to <face_key_register_1>
# face_keys_get_hair = 2652 #(face_keys_get_hair, <destination>, <face_key_register>), #Stores <face_key_register>'s hair value into <destination>
...
face_keys_store_string = 2742 #(face_keys_store_string, <face_key_register>, <string>), #Initializes <face_key_register> using <string>
...
# agent_get_item_modifier = 3330 #(agent_get_item_modifier, <destination>, <agent_no>), #Stores <agent_no>'s horse item modifier (-1 if agent is not a horse) into <destination>
# agent_get_item_slot_modifier = 3331 #(agent_get_item_slot_modifier, <destination>, <agent_no>, <item_slot_no>), #Stores <agent_no>'s <item_slot_no> modifier into <destination>
agent_set_item_slot = 3332 #(agent_set_item_slot, <agent_no>, <item_slot_no>, <item_kind_no>, [<item_modifier_no>]), #Sets <agent_no>'s <item_slot_no> to <item_kind_no> with [<item_modifier_no>]
# agent_get_animation_progress = 3333 #(agent_get_animation_progress, <destination>, <agent_no>, [<channel_no>]), #Stores <agent_no>'s channel [<channel_no>] animation progress (in %%) into <destination>
# agent_get_dna = 3334 #(agent_get_dna, <destination>, <agent_no>), #Stores <agent_no>'s dna into <destination>
...
#agent_get_ground_scene_prop = 3338 #(agent_get_ground_scene_prop, <destination>, <agent_no>), #Stores the prop instance on which <agent_no> is standing into <destination>
agent_get_item_slot_ammo = 3339 #(agent_get_item_slot_ammo, <destination>, <agent_no>, <item_slot_no>), #Stores <agent_no>'s <item_slot_no> ammo count into <destination>
#agent_set_item_slot_ammo = 3340 #(agent_set_item_slot_ammo, <agent_no>, <item_slot_no>, <value>), #Sets <agent_no>'s <item_slot_no> ammo count to <value>
#agent_get_item_slot_hit_points = 3341 #(agent_get_item_slot_hit_points, <destination>, <agent_no>, <item_slot_no>), #Stores <agent_no>'s <item_slot_no> shield hitpoints into <destination>
#agent_set_item_slot_hit_points = 3342 #(agent_set_item_slot_hit_points, <agent_no>, <item_slot_no>, <value>), #Sets <agent_no>'s <item_slot_no> shield hitpoints to <value>
#agent_get_wielded_item_slot_no = 3343 #(agent_get_wielded_item_slot_no, <destination>, <agent_no>, [<hand_no>]), #Stores <agent_no>'s wielded item slot for [<hand_no>] into <destination>
#agent_get_bone_position = 3344 #(agent_get_bone_position, <position_register>, <agent_no>, <bone_no>), #Stores <agent_no>'s <bone_no> position into <position_register>
#agent_get_scale = 3345 #(agent_get_scale, <destination_fixed_point>, <agent_no>), #Stores <agent_no>'s scale into <destination_fixed_point>
#agent_set_forced_lod = 3346 #(agent_set_forced_lod, <agent_no>, <lod_level>), #Forces <agent_no>'s LOD level to <lod_level> (0 = auto)
#WSE chief
# agent_get_item_slot_flags         = 3347 #(agent_get_item_slot_flags, <destination>, <agent_no>, <item_slot_no>), #Stores <agent_no>'s <item_slot_no> item slot flags into <destination>
# agent_ai_get_move_target_position = 3348 #(agent_ai_get_move_target_position, <position_register>, <agent_no>), #Stores <agent_no>'s move target position agent into <position_register>
# gent_get_item_modifier                         = 3300 #(agent_get_item_modifier, <destination>, <agent_no>), #Stores <agent_no>'s horse item modifier (-1 if agent is not a horse) into <destination>
# agent_get_item_slot_modifier                    = 3301 #(agent_get_item_slot_modifier, <destination>, <agent_no>, <item_slot_no>), #Stores <agent_no>'s <item_slot_no> modifier into <destination>
# agent_get_animation_progress                    = 3302 #(agent_get_animation_progress, <destination>, <agent_no>, [<channel_no>]), #Stores <agent_no>'s channel [<channel_no>] animation progress (in %%) into <destination>
# agent_get_dna                                   = 3303 #(agent_get_dna, <destination>, <agent_no>), #Stores <agent_no>'s dna into <destination>
# agent_get_ground_scene_prop                     = 3304 #(agent_get_ground_scene_prop, <destination>, <agent_no>), #Stores the prop instance on which <agent_no> is standing into <destination>
...
# agent_accelerate                                = 3315 #(agent_accelerate, <agent_no>, <position_register_no>), #Uses x, y, z components of <position_register_no> to apply acceleration to <agent_no>
# agent_set_item_slot_modifier                    = 3316 #(agent_set_item_slot_modifier, <agent_no>, <item_slot_no>, <item_modifier_no>), #Sets <agent_no>'s <item_slot_no> modifier to <item_modifier_no>
# agent_body_meta_mesh_set_vertex_keys_time_point = 3317
# multiplayer_send_chat_message_to_player = 3400 #(multiplayer_send_chat_message_to_player, <player_no>, <sender_player_no>, <text>, [<type>]), #Sends <text> to <player_no> as a (native compatible) chat message by <sender_player_no>. Works only on servers. [<type>]: 0 = chat, 1 = team chat
# multiplayer_send_composite_message_to_player = 3401 #(multiplayer_send_composite_message_to_player, <player_no>, <message_type>, <message_register>), #Sends <message_register> with <message_type> to <player_no> (requires network_compatible = 0 in wse_settings.ini)
# multiplayer_send_composite_message_to_server = 3402 #(multiplayer_send_composite_message_to_server, <message_type>, <message_register>), #Sends <message_register> with <message_type> to the server (requires network_compatible = 0 in wse_settings.ini)
# multiplayer_get_cur_profile = 3403 #(multiplayer_get_cur_profile, <destination>), #Stores the current multiplayer profile into <destination>
# multiplayer_get_num_profiles = 3404 #(multiplayer_get_num_profiles, <destination>), #Stores the number of multiplayer profiles into <destination>
multiplayer_profile_get_face_keys = 3435 #(multiplayer_profile_get_face_keys, <face_key_register>, <profile_no>), #Stores <profile_no>'s face keys into <face_key_register>
...
multiplayer_message_put_face_keys = 3431 #(multiplayer_message_put_face_keys, <message_register>, <face_keys_register>), #Puts <face_keys_register> into <message_register> (requires network_compatible = 0 in wse_settings.ini)
...
multiplayer_cur_message_get_face_keys = 3436 #(multiplayer_cur_message_get_face_keys, <face_keys_register>), #Stores face keys from the current message register into <face_keys_register> (requires network_compatible = 0 in wse_settings.ini)
...
lhs_operations += [
	#try_for_agents,###gdw below includes WSE 4.2.4
...
########This was added from Warband Script Enhancer 4.1.0 and has not been intergrated yet gdw
#Add the following definitions to the end (!) of header_operations.py
# server_set_max_num_players   = 491 #(server_set_max_num_players, <max_players>, [<max_private_players>]), #Sets maximum players to <max_players> and maximum private players to [<max_private_players>] (default = same as <max_players>). Both values must be in the range 2-250, [<max_private_players>] can't be lower than <max_players>
# position_rotate_x            = 723 #(position_rotate_x, <position_register>, <angle>, [<use_global_axis>]), #Rotates <position_register> around the x-axis by <angle> degrees
# position_rotate_y            = 724 #(position_rotate_y, <position_register>, <angle>, [<use_global_axis>]), #Rotates <position_register> around the y-axis by <angle> degrees
