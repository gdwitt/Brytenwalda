       ...
       [
        ...
        (try_begin),
          ...
          (try_begin), #server stop reinforcing and be ready for next scene
            ...
            (assign, ":reserves", 20), #number to reserve for hall battle
            (try_begin),
              (gt, "$coop_street_scene", 0),
              (eq, "$coop_round", coop_round_battle),
              (assign, ":reserves", 80), #if street battle is comming
            (try_end),
 #          (store_div, ":reserves", "$coop_battle_size", 6), #use if reserves should scale with battle size

            (try_begin),
              (eq, "$defender_team", 0),
              (le, "$coop_num_bots_team_1", ":reserves"),
        ...
        #    (agent_get_troop_id, ":troop_no", ":agent_no"),
            ...
            (try_begin),
              ...
              (try_begin),
                ...
                (else_try),
                  (eq, "$coop_round", coop_round_town_street),
                  (assign, ":num_row", 10), #10 per row in street
                (else_try),
...
###
        ...
        (try_begin), #check round end        
          ...
          (try_begin),
            ...
            (assign, "$coop_winner_team", 1),
            (try_begin),
              (ge, "$coop_num_bots_team_1", 20), #if loser has reserves, they retreated
          ...
          (else_try),
            ...
            (assign, "$coop_winner_team", 0),
            (try_begin),
              (ge, "$coop_num_bots_team_2", 20), #if loser has reserves, they retreated
              (display_message, "@The defenders retreat!"),
            (try_end),
          (try_end),
