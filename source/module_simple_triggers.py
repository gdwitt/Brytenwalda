...
# This trigger will check if player's raid has been completed and will lead control to village menu.
  ...
  #cabagdw per http://forums.taleworlds.com/index.php/topic,148290.msg3568597.html#msg3568597
  (0.25,
   [
      (ge,"$g_player_raiding_village",1),
...
##    ]),
  ...
  (24, #chief pone 48 horas a spawn de campesinos en vez de 24#gdw up from 48
   [
       (try_for_range, ":village_no", villages_begin, villages_end),
...
#Recruiter kit end
 ...
 [
  ...
  # Constable training
   ...
   (store_skill_level, ":training_points", skl_trainer, "trp_player"), # TML constable script rewrite begin. F123 - Submod -> 1.41
   #(val_add, ":trainer_level", 100), #chief cambia porque las tropas requieren mas experiencia en brytenwalda. - What? Below the xp_gain is used as max # of troops trained, if this script actually worked that change would have it training over 200 troops each time. Yikes. Whoever edited this script must not have understood it. - TML
   (val_mul, ":training_points", 10),
   (val_add, ":training_points", 100), # TML Base of 100 + 10 per level of the player's base trainer skill, max of 200 points at 10 skill.
   (try_begin),
     (eq, "$g_constable_training_improved", 1), # TML 50% extra training points if you are using improved training.
   ...
   # (eq, ":party_no", "$g_constable_training_center"),
    ...
    (try_for_range, ":i_stack", 0, ":num_stacks"),
      (eq, ":trained", 0),
      (party_stack_get_troop_id, ":troop_id", "$g_constable_training_center", ":i_stack"),#TML changes endabove
      ...
      (store_character_level, ":troop_level", ":troop_id"),#gdw>onlyTML changes below
      (assign, ":point_cost", 0),
      (try_begin),
        (is_between, ":troop_level", 0, 16),
        (assign, ":point_cost", 10), # TML 10 for basic troops, so max you can train per day is 10-30 depending on your trainer skill and whether you use improved training. F123 - Submod -> 1.41
      (else_try),  
        (is_between, ":troop_level", 16, 20),
        (assign, ":point_cost", 12), # TML 12 for 2nd tier troops, so max you can train per day is 8-25 depending on your trainer skill and whether you use improved training.
      (else_try),  
        (is_between, ":troop_level", 20, 24),
        (assign, ":point_cost", 20), # TML 20 for 3rd tier troops, so max you can train per day is 5-15 depending on your trainer skill and whether you use improved training.
      (else_try),  
        (is_between, ":troop_level", 24, 30),
        (assign, ":point_cost", 50), # TML 50 for 4th tier troops, so max you can train per day is 2-6 depending on your trainer skill and whether you use improved training.
        (try_begin),
          (eq, "$g_realism_upgrade", 0), # TML What realism upgrade actually does is make the last tier upgrade take 50% longer specifically for the player, so I figured I should make it consistent on constable training. 
          (assign, ":point_cost", 75),
        (try_end),
      (else_try),
        (assign, ":point_cost", 100), # TML 100 for anything else, just in case I or someone adds higher level troops.
      (try_end),
      ...
      #  (try_begin),
      #    (le, ":troop_level", 22), #chief cambia
      #    (val_add, ":xp_gain", 20), #more recruits are trained during improved training #chief cambia porque tropas necesitan mas experiencia en brytenwalda
      #  (try_end),
      #(try_end),
      ...
      (try_begin),
        (eq, "$g_constable_training_improved", 1),
        (val_mul, ":upgrade_cost", 11), # TML +10% cost if using improved training.
      ...
      (str_store_party_name, s8, "$g_constable_training_center"), # TML constable script rewrite end. F123 - Submod -> 1.41
