...
[
   ...
   # (try_begin),this_or_next|
    (this_or_next|ge, ":cur_hour", 20),
    (le, ":cur_hour", 1),
    (assign, reg10,":cur_hour"),
    (set_show_messages, 1),
    (store_current_hours, reg9),
    #(display_message, "@{!}DEBUG missed poltical call at {reg10}and {reg9}"),##if it gets to hear it nmeans it passeed the checks above it
   # (else_try),
   #  (assign, reg10,":cur_hour"),
   #  (display_message, "@{!}DEBUG missed call at {reg10}and {reg9}",0xFF0000),
   #  (set_show_messages, 1),
    # (store_current_hours, reg9),
  ],
...
##chief anadido acaba
    ...
    (try_begin),
        (ge, "$cheat_mode", 0),
        (display_message, "@{!}DEBUG -- Doing updating faction notes for triggers at {reg10} and {reg9}"),
    (try_end),
  (try_end),
...
# Refresh Horse sellers
  ...
  [
    ...
    (display_message,"@{!}refresh_special_merchants1"),
  ]),
  # Refresh bandit merchants
  (0, 0, 24 * 14, [],
  [
    (call_script, "script_refresh_special_merchants2"),
    (display_debug_message,"@refresh_special_merchants2"),
  ]),
  # Refresh bandit merchants
  (0, 0, 24 * 14, [],
  [
    (call_script, "script_refresh_special_merchants3"),
  ]),
  # Refresh bandit merchants
  # (24, 0, 24 * 14, [],
  # [
  #   (call_script, "script_refresh_special_merchants4"),
  # ]),
  # (24,0, 24 * 14, [],
  # [
  #   (call_script, "script_refresh_special_merchants5"),
  # ]),
  # #gdw these can be used as lost for other triggers
  # (24, 0, 24 * 14, [],
  # [
  #   (call_script, "script_refresh_special_merchants6"),
  # ]),
 
 # (0.0, 0, 368, [],
 #  [
 #    (call_script, "script_refresh_special_merchants7"),
 #  ]),
 # (0.0, 0, 368.0, [],
 #  [
 #    (call_script, "script_refresh_special_merchants6"),
 ...
 #  [
 #    (call_script, "script_refresh_special_merchants6"),
 #    #(call_script, "script_log_item_score_for_weapons"),
 #  ]),
